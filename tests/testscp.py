...
class TestEverything(unittest.TestCase):
    ...
    x_small = cp.array((0.0, 1.0, 1.5, 2.0))
    y_small = cp.array((0.0, 1.0, 1.1, 1.5))
    x_sin = cp.linspace(0, 10, num=100)
    y_sin = cp.sin(x_sin * cp.pi / 2)

    def test_break_point_spot_on(self):
        # check that I can fit when break points spot on a
        my_fit1 = pwlf.PiecewiseLinFitCp(self.x_small, self.y_small)
        x0 = self.x_small.copy()
        ssr = my_fit1.fit_with_breaks(x0)
        self.assertTrue(cp.isclose(ssr, 0.0))

    def test_break_point_spot_on_list(self):
        # check that I can fit when break points spot on a
        my_fit1 = pwlf.PiecewiseLinFitCp(list(self.x_small), list(self.y_small))
        x0 = self.x_small.copy()
        ssr = my_fit1.fit_with_breaks(x0)
        self.assertTrue(cp.isclose(ssr, 0.0))
    ...
    def test_degree_not_supported(self):
        try:
            _ = pwlf.PiecewiseLinFitCp(self.x_small, self.y_small,
                                     degree=100)
    ...
    def test_x_c_not_supplied(self):
        try:
            mf = pwlf.PiecewiseLinFitCp(self.x_small, self.y_small,
                                      degree=1)
            mf.fit(2, x_c=[1.0])
            self.assertTrue(False)
        except ValueError:
    ...
    def test_assembly(self):
        # check that I can fit when break points spot on a
        my_fit = pwlf.PiecewiseLinFitCp(self.x_small, self.y_small)
        x0 = self.x_small.copy()
        A = my_fit.assemble_regression_matrix(x0, my_fit.x_data)
        Asb = cp.array([[1.,  0.,  0.,  0.],
                        [1.,  x0[1]-x0[0],  0.,  0.],
                        [1.,  x0[2]-x0[0], x0[2]-x0[1], 0.],
                        [1.,  x0[3]-x0[0], x0[3]-x0[1], x0[3]-x0[2]]])
        self.assertTrue(cp.allclose(A, Asb))
    ...
    def test_break_point_spot_on_r2(self):
        ...
        self.assertTrue(cp.isclose(rsq, 1.0))

    def test_break_point_diff_x0_0(self):
        # check diff loc
        my_fit2 = pwlf.PiecewiseLinFitCp(self.x_small, self.y_small)
        x0 = self.x_small.copy()
        x0[1] = 1.00001
        x0[2] = 1.50001
        ssr2 = my_fit2.fit_with_breaks(x0)
        self.assertTrue(cp.isclose(ssr2, 0.0))

    def test_break_point_diff_x0_1(self):
        # check if my duplicate is in a different location
        x0 = self.x_small.copy()
        my_fit3 = pwlf.PiecewiseLinFitCp(self.x_small, self.y_small)
        x0[1] = 0.9
        ssr3 = my_fit3.fit_with_breaks(x0)
        self.assertTrue(cp.isclose(ssr3, 0.0))

    def test_break_point_diff_x0_2(self):
        # check if my duplicate is in a different location
        x0 = self.x_small.copy()
        my_fit4 = pwlf.PiecewiseLinFitCp(self.x_small, self.y_small)
        x0[1] = 1.1
        ssr4 = my_fit4.fit_with_breaks(x0)
        self.assertTrue(cp.isclose(ssr4, 0.0))

    def test_break_point_diff_x0_3(self):
        # check if my duplicate is in a different location
        x0 = self.x_small.copy()
        my_fit5 = pwlf.PiecewiseLinFitCp(self.x_small, self.y_small)
        x0[2] = 1.6
        ssr5 = my_fit5.fit_with_breaks(x0)
        self.assertTrue(cp.isclose(ssr5, 0.0))

    def test_break_point_diff_x0_4(self):
        # check if my duplicate is in a different location
        x0 = self.x_small.copy()
        my_fit6 = pwlf.PiecewiseLinFitCp(self.x_small, self.y_small)
        x0[2] = 1.4
        ssr6 = my_fit6.fit_with_breaks(x0)
        self.assertTrue(cp.isclose(ssr6, 0.0))

    def test_diff_evo(self):
        my_pwlf = pwlf.PiecewiseLinFitCp(self.x_small, self.y_small)
        my_pwlf.fit(4, disp=False)
        self.assertTrue(cp.isclose(my_pwlf.ssr, 0.0))

    def test_custom_bounds1(self):
        my_pwlf = pwlf.PiecewiseLinFitCp(self.x_small, self.y_small)
        n_segments = 3
        bounds = cp.zeros((n_segments-1, 2))
    ...
    def test_predict(self):
        my_pwlf = pwlf.PiecewiseLinFitCp(self.x_small, self.y_small)
        xmax = cp.max(self.x_small)
        xmin = cp.min(self.x_small)
        my_pwlf.fit_with_breaks((xmin, xmax))
        x = cp.linspace(xmin, xmax, 10)
        yHat = my_pwlf.predict(x)
        self.assertTrue(cp.isclose(cp.sum(yHat), 8.085714285714287))

    def test_custom_opt(self):
        my_pwlf = pwlf.PiecewiseLinFitCp(self.x_small, self.y_small)
        my_pwlf.use_custom_opt(3)
        x_guess = cp.array((0.9, 1.1))
        from scipy.optimize import minimize
        res = minimize(my_pwlf.fit_with_breaks_opt, x_guess)
        self.assertTrue(cp.isclose(res['fun'], 0.0))
    ...
    def test_single_force_break_point1(self):
        ...
        self.assertTrue(cp.isclose(y_c, yhat))
    ...
    def test_single_force_break_point_degree(self):
        my_fit = pwlf.PiecewiseLinFitCp(self.x_small, self.y_small, degree=2)
        x_c = [2.0]
        y_c = [1.5]
        my_fit.fit_with_breaks_force_points([0.2, 0.7], x_c, y_c)
        yhat = my_fit.predict(x_c)
    ...
    def test_single_force_break_point_degree_zero(self):
        my_fit = pwlf.PiecewiseLinFitCp(self.x_small, self.y_small, degree=0)
        x_c = [2.0]
        y_c = [1.5]
        my_fit.fit_with_breaks_force_points([0.2, 0.7], x_c, y_c)
        yhat = my_fit.predict(x_c)
    ...
    def test_opt_fit_with_force_points(self):
        ...
        x = cp.linspace(0.0, 1.0, num=100)
        y = cp.sin(6.0*x)
        my_fit = pwlf.PiecewiseLinFitCp(x, y, disp_res=True)
        x_c = [0.0]
        y_c = [0.0]
    ...
    def test_se(self):
        ...
        my_pwlf = pwlf.PiecewiseLinFitCp(cp.random.random(20),
                                       cp.random.random(20))
        my_pwlf.fitfast(2)
    ...
    def test_nonlinear_p_and_se(self):
        ...
        cp.random.seed(1)
        n_data = 20
        x = cp.linspace(0, 1, num=n_data)
        y = cp.random.random(n_data)
        my_pwlf = pwlf.PiecewiseLinFitCp(x, y)
        true_beta = cp.array((1.0, 0.2, 0.2))
        true_breaks = cp.array((0.0, 0.5, 1.0))
    ...
    def test_pv(self):
        ...
        my_pwlf = pwlf.PiecewiseLinFitCp(cp.random.random(20),
                                       cp.random.random(20))
        my_pwlf.fitfast(2)
        my_pwlf.prediction_variance(cp.random.random(20))

    def test_predict_with_custom_param(self):
        # check to see if predict runs with custom parameters
        x = cp.random.random(20)
        my_pwlf = pwlf.PiecewiseLinFitCp(x, cp.random.random(20))
        my_pwlf.predict(x, beta=cp.array((1e-4, 1e-2, 1e-3)),
                        breaks=cp.array((0.0, 0.5, 1.0)))
    ...
    def test_fit_guess(self):
        ...
        self.assertTrue(cp.isclose(breaks[1], 6.0705297))
    ...
    def test_n_parameters_correct(self):
        my_pwlf_0 = pwlf.PiecewiseLinFitCp(self.x_sin, self.y_sin, degree=0)
        my_pwlf_1 = pwlf.PiecewiseLinFitCp(self.x_sin, self.y_sin, degree=1)
        my_pwlf_2 = pwlf.PiecewiseLinFitCp(self.x_sin, self.y_sin, degree=2)
        breaks = cp.array([0.,  1.03913513,  3.04676334,  4.18647526, 10.])
    ...
    def test_force_fits_through_points_other_degrees(self):
        ...
        x = cp.linspace(0, 10, num=100)
        y = cp.sin(x * cp.pi / 2)
        # add noise to the data
        y = cp.random.normal(0, 0.15, 100) + y

        # linear fit
        my_pwlf_1 = pwlf.PiecewiseLinFitCp(x, y, degree=1)
        my_pwlf_1.fit(n_segments=6, x_c=[0], y_c=[0])
        y_predict_1 = my_pwlf_1.predict(x)

        # quadratic fit
        my_pwlf_2 = pwlf.PiecewiseLinFitCp(x, y, degree=2)
        my_pwlf_2.fit(n_segments=5, x_c=[0], y_c=[0])
        y_predict_2 = my_pwlf_2.predict(x)
        self.assertTrue(cp.isclose(0, y_predict_1[0]))
        self.assertTrue(cp.isclose(0, y_predict_2[0]))
    ...
    def test_se_no_fit(self):
        my_pwlf_0 = pwlf.PiecewiseLinFitCp(self.x_sin, self.y_sin)
        try:
    ...
    def test_all_stats(self):
        ...
        my_pwlf_0.fitfast(3)
        my_pwlf_0.standard_errors()
        my_pwlf_0.prediction_variance(cp.random.random(20))
        my_pwlf_0.p_values()
        my_pwlf_0.r_squared()
        ...
        my_pwlf_2.prediction_variance(cp.random.random(20))
        my_pwlf_2.p_values()
        my_pwlf_2.r_squared()
        my_pwlf_2.calc_slopes()

        my_pwlf_3 = pwlf.PiecewiseLinFitCp(self.x_sin, self.y_sin, degree=3)
        my_pwlf_3.fitfast(3)
        my_pwlf_3.standard_errors()
        my_pwlf_3.prediction_variance(cp.random.random(20))
        my_pwlf_3.p_values()
        my_pwlf_3.r_squared()
